...
class InterpolActor(Actor):
        ...
        def __interpolate(self):
                ...
                if (self.__startInterp == True):
                        ...
                        i = int(elapsedTime) + 1
                        # interpol = []
                        # interpol = self.__interpol_points[i]
                        # print str(interpol[0]) + ', ' + str(interpol[1]) + ', ' + str(interpol[2])
                        ...
                        if (self.__interpOper & self.POSITION) != 0 :
                                ...
                                currentPosition = self.__position + (Vector3(self.__interpol_points[i][0], self.__interpol_points[i][1], self.__interpol_points[i][2]) - self.__position) * smoothFunction
...
##########################################################################################################
        ...
        def generatecircularOrbit(self,latstart,lonstart,latend,lonend):
                self.__interpol_points = []
                latinterval = None
                loninterval = None
                distance = None
                distanceInterval = 0
                ...
                start = geodetic2ecef(latstart, lonstart, 600)
                target = geodetic2ecef(latend, lonend, 600)

                # height is a helper to create a semi-circular path from Start-point to the Destination. It increments and then decrements by equal amount.
                distance = sqrt(((start[0] - target[0]) * (start[0] - target[0])) + ((start[1] - target[1]) * (start[1] - target[1])) + ((start[2] - target[2]) * (start[2] - target[2])))
                distance /= 2
                
                tempduration = self.__duration / 2

                # distanceInterval = distance / tempduration
                distanceInterval = 1000
                height = distanceInterval


                # print "Generating Points for " + str(latstart) + ',' + str(latend) + ',' + str(lonstart) + ',' + str(lonend)

                latinterval = (latend - latstart) / self.__duration
                loninterval = (lonend - lonstart) / self.__duration

                latlist.append(latstart)
                lonlist.append(lonstart)
                heightlist.append(distanceInterval)

                currLat = latstart
                currLon = lonstart

                for indx in range(self.__duration):
                        currLat += latinterval
                        latlist.append(currLat)
                        currLon += loninterval
                        lonlist.append(currLon)
                        if(indx < tempduration):
                                height += distanceInterval
                                heightlist.append(height)
                        if(indx == tempduration):
                                heightlist.append(height)
                        elif(indx > tempduration):
                                height -= distanceInterval
                                heightlist.append(height)
                ...
                for index in range(len(latlist)):
                        cartlist = geodetic2ecef(latlist[index], lonlist[index], heightlist[index])
                        self.__interpol_points.append(cartlist)

                # print 'target Position is ' + str(self.__targetPosition) + 'and last point generated is' + str(cartlist)
                self.__interpol_points.append(self.__targetPosition)

                # print 'total number of points in self.__interpol_points is ' + str(len(self.__interpol_points))
##########################################################################################################
